    ...
    .then((config) => {
      ...
      if ($.isEmptyObject(config)) {
        ...
        if (config.tabGroups) {
          ...
          $switcher.append(
            `<div class="choice-option" id="select0" onclick='selectGroup(0, ${numGroups}, this)'>Playground</div>`
          );
          for (let i = 0; i < numGroups; i++) {
            let group = config.tabGroups[i];
            $switcher.append(
              `<div class="choice-option" id='select${
                i + 1
              }' onclick='selectGroup(${i + 1}, ${numGroups}, this)'> ${
                group.name
              } </div>`
            );
          }
        ...
        if (config.authentication) {
          const { logout_endpoint, userinfo_endpoint } = config.authentication;

          fetch(logout_endpoint)
            .then((res) => res.json())
            .then((res) => {
              const { outer, inner } = res;

              const logout_url = `${outer}${encodeURIComponent(
                inner + window.location.origin
              )}`;

              fetch(userinfo_endpoint)
                .then((res) => res.json())
                .then((res) => {
                  const { email, name, preferred_username } = res;
                  ...
                  if (display_name) {
                    $("#info").prepend(
                      `<div style="text-align: right;">
                        ...
                        <span>|</span>
                        <a href="${logout_url}" class="yasgui">Logout</a>
                      </div>`
                    );
                  } else {
                    $("#info").prepend(
